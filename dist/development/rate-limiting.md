...
### [CRITICAL] In-memory rate limiting doesn't work in production

**Situation:** You implement rate limiting with a Map or in-memory store. Works in dev.
...
### [HIGH] Fixed window allows 2x limit at window boundaries

**Situation:** Your limit is 100 requests per minute. User makes 100 requests at 11:59:59.
...
### [HIGH] IP-only limiting fails for shared IPs

**Situation:** You rate limit by IP address. A large company (thousands of employees)
...
### [HIGH] Missing Retry-After header causes client storms

**Situation:** Your API returns 429 without Retry-After header. Well-behaved clients
...
### [MEDIUM] Rate limiting health check endpoints breaks orchestration

**Situation:** You apply rate limiting to all routes. Health check endpoint gets
...
### [MEDIUM] Blocking users without warning escalates

**Situation:** User hits rate limit. You immediately block for an hour. User gets
...
### [MEDIUM] Race condition in Redis rate limiting

**Situation:** Your rate limit code does: GET count, check limit, INCR count.
...
### [MEDIUM] Can't test API because rate limits block tests

**Situation:** Your test suite runs 1000 API calls. Rate limit is 100/minute.
...
|---------|-------------|--------|
| `redis|cache|store` | caching-patterns | Need Redis for distributed rate limiting |
| `monitor|metrics|alert` | observability-sre | Need rate limit monitoring |
| `user tier|plan|pricing` | backend | Need user plan information for tiered limits |
| `cloudflare|waf|edge` | infrastructure-as-code | Need edge rate limiting |
| `block|ban|abuse` | security-hardening | Need abuse prevention |

### Receives Work From

- **backend**: Backend needs API rate limiting
- **authentication-oauth**: Auth endpoints need brute-force protection
- **queue-workers**: Workers need rate limiting for external APIs
- **api-designer**: API needs abuse protection
- **security-hardening**: Security needs DDoS/abuse protection
...
Full skill path: `~/.spawner/skills/development/rate-limiting/`
